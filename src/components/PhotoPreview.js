...
const PhotoPreview = ({ capturedImages: initialImages }) => {
	...
	const [customizeTab, setCustomizeTab] = useState('colors'); // 'colors', 'frames'
			...
			<div className="customization-section" style={{
				...
				{customizeTab === 'frames' && (
					<>
						...
						{isMobile && (
							<div 
								className="mobile-frames-container"
								ref={framesContainerRef}
								style={{
									overflowX: "auto",
									overflowY: "hidden",
									WebkitOverflowScrolling: "touch", // 使滚动在iOS上更平滑
									scrollbarWidth: "none", // 隐藏Firefox滚动条
									msOverflowStyle: "none", // 隐藏IE滚动条
									paddingBottom: "5px" // 为滚动指示器留出空间
								}}
							>
								<div style={{
									display: "grid",
						...
						{isMobile && (
							<div style={{
								...
								<div style={{
									...
									<span style={{ fontSize: "14px" }}>←</span>
									<span>Swipe to see more frames</span>
									<span style={{ fontSize: "14px" }}>→</span>
								</div>
							</div>
						)}
	...
	return (
		<>
			...
			{/* CSS for drag and drop and mobile interactions */}
			<style>
				{`
					.photo-item {
			...
			<div className="photo-preview" style={{ 
				...
				{/* 主要内容区 - PC端使用双列布局，移动端始终显示预览 */}
				<div style={{ 
					display: "flex", 
					flexDirection: isMobile ? "column" : "row",
					gap: "20px",
					position: "relative",
					justifyContent: "center" // 确保整体内容居中
				}}>
					{/* 左侧预览区 */}
					<div style={{ 
						flex: isMobile ? "1" : "0 0 auto",
						alignSelf: "center" // 确保预览区域居中
					}}>
						<div className="preview-container">
							<canvas ref={stripCanvasRef} className="photo-strip" style={{
								maxWidth: isMobile ? "100%" : "350px", // 在PC端适当缩小预览图片
								height: "auto",
